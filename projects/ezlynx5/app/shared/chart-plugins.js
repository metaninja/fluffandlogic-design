...
var addDataTogeather = {
    afterDatasetsDraw: function(chart, easing) {
        ...
        chart.data.datasets.forEach(function (dataset, i) {
            ...
            if (!meta.hidden) {
                meta.data.forEach(function(element, index) {
                    var dataAmount = dataset.data[index];
                    var position = element.tooltipPosition();

                    total[index] = {
                        amount: total[index] ? total[index].amount + dataAmount : dataAmount,
                        position: position
                    };
                });
            }
        ...
        total.forEach(function(data) {
            ctx.fillText(data.amount, data.position.x, data.position.y - (chartStyles.fontSize / 2) - chartStyles.padding);
        });
    }
};
