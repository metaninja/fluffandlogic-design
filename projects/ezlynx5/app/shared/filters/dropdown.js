...
$(document).ready(function() {
    ...
    function addFilter(node) {
        ...
        $('.filters-applied').append('<span class="badge badge-primary mr-1">' + label + ": " + node.val() + ' <i class="linearicon-cross filter-remove"></i></span>');

        $('.filter-remove').click(function() {
            $(this).parents('.badge').remove();
        });

        node.parents('.filter-dropdown').removeClass('active');
    }

    $('.filter-dropdown input').keyup(function(e) {
        if (e.keyCode === 13) {
            addFilter($(this));
        }
    });
