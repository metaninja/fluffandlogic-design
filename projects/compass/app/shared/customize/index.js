...
function setBackground() {
    ...
    if (backgroundImageSrc === 'default' || backgroundImageSrc === undefined) {
        $(".main-topnav").css('background-image', 'url()');
        $(".main-topnav").removeClass("custom-background");
    } else {
...
function updateTopnavBackground() {
    ...
    if ($imageContainer.hasClass("default-background-image")) {
...
function setAgentPhoto() {
    var agentPhotoSrc = localStorage.topnavAgentPhoto || "libs/images/agent-photos/agent-photo-1.jpg";
...
function getAndSetAllBackgroundImages() {
    customBackgroundImages.forEach(function(value) {
        ...
        if (value === "default") {
            $(".top-nav-background-images-container").append("<li class=default-background-image></li>");
        } else {
            var $li = $("<li></li>");
...
$(function() {
    ...
    $(".js-customize-modal .tab-link").on("click", function() {
        ...
        if (val === "Backgrounds") {
            $(".js-backgrounds-tab-content").show();
        } else if (val === "Agent Photos") {
            $(".js-agent-photos-tab-content").show();
        }
    });
